...
public class DemoApplication extends Application {
	...
	public void initEngineManager(Context context) {
        ...
        if (!mBMapManager.init(new MyGeneralListener())) {
            ...
            Log.e("dengying", "BMapManager  初始化错误!");
        }
	}
    ...
    static class MyGeneralListener implements MKGeneralListener {
        ...
        public void onGetPermissionState(int iError) {
            ...
            if (iError != 0) {
                //授权Key错误：
                Log.e("dengying", "AndroidManifest.xml 文件输入正确的授权Key,并检查您的网络连接是否正常！error: "+iError);
                //Toast.makeText(DemoApplication.getInstance().getApplicationContext(), "AndroidManifest.xml 文件输入正确的授权Key,并检查您的网络连接是否正常！error: "+iError, Toast.LENGTH_LONG).show();
                DemoApplication.getInstance().m_bKeyRight = false;
            }
            else{
            	Log.e("dengying", "key认证成功 ");
